	...
	<div class="pagewrapper clearfix" id="profile" >
		...
		<aside class="pagesidebar" style="top:44px;">
			<ul class="pagesider-menu">
				...
				<li class="<?php if(isset($_GET["pd"]) && $_GET['pd'] == 'ad') echo 'active';?>"><a href="?pd=ad">我的广告</a></li>
				<?php }?>
				<?php if(function_exists('erphpdown_tuan_install')){?>
				<li class="<?php if(isset($_GET["pd"]) && $_GET['pd'] == 'tuan') echo 'active';?>"><a href="?pd=tuan">我的拼团</a></li>
				<?php }?>
		...
		<div class="pagecontent profile-content">
			...
			?>
				<div class="profile-form">
					...
					<dl class="dl-horizontal">
						<dt>提现金额</dt>
						<dd>
							<input type="text" class="profile-input" id="ice_money" name="ice_money"  />（可提现余额：<?php echo sprintf("%.2f",$okMoney)?><?php echo get_option('ice_name_alipay')?>）
						</dd>
					</dl>
					<dl class="dl-horizontal">
						<dt></dt>
			...
			<?php }elseif(isset($_GET["pd"]) && $_GET["pd"]=='money'){ 
                            ...
                            <tr>
			            		...
			            		?>
									...
									<?php if(get_option('erphpdown_epay_id')){?>
										...
										<?php if(!get_option('erphpdown_epay_qqpay')){?><input type="radio" id="paytype23" class="paytype" name="paytype" value="23" checked/>QQ钱包&nbsp;<?php }?>
                ...
                <style>.form-table del{font-size: 12px;margin-right: 5px;}</style>
                	<table class="form-table">
                        <tr>
                            <td><?php 
							$erphp_year_price    = get_option('ciphp_year_price');
                        ...
                        <tr>
                            <td>类型：
                                <?php if($erphp_life_price){?><input type="radio" id="userType10" name="userType" value="10" checked /><?php echo $erphp_life_name;?>(<?php
				                    if($vip_update_pay){
			             		...
			             		?><?php echo get_option('ice_name_alipay');if($erphp_life_days) echo '&nbsp;'.$erphp_life_days.'年';?>)<?php }?>&nbsp;
                                <?php if($erphp_year_price){?><input type="radio" id="userType9" name="userType" value="9" checked/><?php echo $erphp_year_name;?>(<?php
				                    if($vip_update_pay){
				                    	if($userTypeId == 6 && $erphp_day_price){
			             		...
			             		?><?php echo get_option('ice_name_alipay');if($erphp_year_days) echo '&nbsp;'.$erphp_year_days.'个月';?>)&nbsp;<?php }?>
                                <?php if($erphp_quarter_price){?><input type="radio" id="userType8" name="userType" value="8" checked/><?php echo $erphp_quarter_name;?>(<?php
				                    if($vip_update_pay){
				                    	if($userTypeId == 6 && $erphp_day_price){
			             		...
			             		?><?php echo get_option('ice_name_alipay');if($erphp_quarter_days) echo '&nbsp;'.$erphp_quarter_days.'个月';?>)&nbsp;<?php }?>
                                <?php if($erphp_month_price){?><input type="radio" id="userType7" name="userType" value="7" checked/><?php echo $erphp_month_name;?>(<?php
				                    if($vip_update_pay){
				                    	if($userTypeId == 6 && $erphp_day_price){
			             		...
			             		?><?php echo get_option('ice_name_alipay');if($erphp_month_days) echo '&nbsp;'.$erphp_month_days.'天';?>)&nbsp;<?php }?>
                                <?php if($erphp_day_price){?><input type="radio" id="userType6" name="userType" value="6" checked/><?php echo $erphp_day_name;?>(<?php echo $erphp_day_price.get_option('ice_name_alipay');if($erphp_day_days) echo '&nbsp;'.$erphp_day_days.'天';?>)<?php }?>
                            </td>
                        </tr>
                        <tr>
                            <td>方式：
